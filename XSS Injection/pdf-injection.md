...
## Sample
要在 Linux 终端中生成 PDF 文件，可以使用 Ghostscript 和 LaTeX 等工具。下面是一个使用 LaTeX 生成 PDF 文件的示例：

安装 LaTeX 和 Ghostscript
在 Ubuntu 系统中，可以使用以下命令安装 LaTeX 和 Ghostscript：

```bash
sudo apt-get install texlive-latex-base ghostscript
```
创建 LaTeX 源文件
在终端中创建一个名为 example.tex 的文件，输入以下内容：
...
\end{document}
```
这是一个简单的 LaTeX 文件，用于生成包含“Hello, LaTeX!”文本的 PDF 文件。

使用 LaTeX 编译源文件
在终端中使用以下命令编译 LaTeX 源文件：

```bash
pdflatex example.tex
```
这会生成一个名为 example.pdf 的文件，其中包含了生成的 PDF 文件。

在 PDF 文件中插入恶意脚本
在终端中使用文本编辑器打开 example.pdf 文件，将以下内容添加到文件的结尾：
...
endobj
4 0 obj
<< /Type /Page /Parent 3 0 R /Resources << /XObject << /Im1 5 0 R >> >> /Contents 6 0 R >>
endobj
5 0 obj
<< /Type /XObject /Subtype /Image /Name /Im1 /Filter [/ASCIIHexDecode] /Length 10 >>stream
3C7363726970743E616C65727428274A534F4E2D6578636574652827293B3C2F7363726970743E
endstream
endobj
6 0 obj
<< /Length 39 >>
...
/F1 12 Tf
1 0 0 1 72 720 Tm
(Hello, <script>alert('XSS')</script>!) Tj
ET
Q
endstream
endobj
...
%%EOF
```
在文件的结尾处添加的是一个 PDF 文件的最小结构，其中包含了一个名为 Im1 的图片对象和一个名为 F1 的字体对象，以及一个包含恶意脚本的文本对象。

保存文件并查看效果
保存文件后，在终端中使用以下命令打开 PDF 文件：

```
evince example.pdf
```
这会在文本中显示“Hello, LaTeX!”文本，同时也会弹出一个对话框，显示“XSS”弹框。这就是一个简单的 PDF 文件中存在 XSS 漏洞的示例。
